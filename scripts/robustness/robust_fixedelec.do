/*
 Robustness checks: do results change if we restrict elections to fixed ones?
 */
...
xtabond2 $y $x $politvars y1-y12 $macrolags $bankvars, ///
ivstyle($x $politvars y1-y12 $macrolags) gmmstyle($bankvars, ///
laglimits(. 2) collapse) robust noleveleq small
eststo rl4
estadd local estmethod "AB"
estadd local yearfe "Yes"
estadd local countryfe "Yes"
...
#delimit ;
estout baseline2 baseline3 baseline4 cb2 cb3 cb4 ce2 ce3 ce4 rl2 rl3 rl4 using ./output/tables/robust_fixed.tex,
			style(tex)
